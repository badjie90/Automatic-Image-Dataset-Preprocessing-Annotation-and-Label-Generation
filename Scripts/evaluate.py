...
def predict(model,row,device,reference='confidence_filtered'):
    target_path=DATA/'dense_proposals'/f"{row['image_id']}.png" if reference=='dense' else DATA/row['mask']
    rgb=np.array(Image.open(DATA/row['image']).convert('RGB'));target=np.array(Image.open(target_path))
...
def export_model(model,temperature,rows,device):
    ...
    report={'onnx_file':str(path.relative_to(ROOT)),'opset':13,'input_name':'rgb','input_shape':[1,3,384,640],'input_dtype':'float32',
            ...
            'source_checkpoint_sha256':hashlib.sha256((RUN/'best.pt').read_bytes()).hexdigest(),'temperature':temperature}
...
def main():
    ...
    ap.add_argument('--reference',choices=['dense','confidence_filtered'],default='dense')
    ap.add_argument('--reuse-export',action='store_true',help='Reuse an existing verified export only if checkpoint, temperature, and ONNX hashes match')
    ...
    for i,row in enumerate(test):
        logits,rgb,target=predict(model,row,args.device,args.reference);p0=logits.softmax(0).cpu().numpy();p=(logits/temp).softmax(0).cpu().numpy()
        before.update(p0,target,with_boundary=False);after.update(p,target);one=Metrics();one.update(p,target,with_boundary=False)
        per_image.append({'image_id':row['image_id'],'reference_type':'automatic_pseudo_labels','reference_policy':args.reference,'valid_pixel_fraction':float((target<3).mean()),**{k:v for k,v in one.summary().items() if not k.startswith('BF1')}});full_pixels+=target.size
        if i in qualitative:
    ...
    common={'model':'GCNet-S','checkpoint_epoch':ck['epoch']+1,'split':'test','reference_type':'automatic_pseudo_labels','route_independence_verified':False,
            'human_reviewed':False,'reference_policy':args.reference,'test_images':len(test),'evaluated_pixel_fraction':after.n/full_pixels,'ECE_bins':15,'BF1_tolerance_pixels':3,
    ...
    (out/'protocol.json').write_text(json.dumps({**common,'ECE':'top-label confidence, 15 equal-width bins, global pixel weighting, confidence 1 included in last bin',
        ...
        'caveat':'agreement and calibration relative to automatically generated labels; dense references retain uncertain boundary pixels and exclude ego/car-mount; confidence-filtered references remove most boundaries and are unsuitable for interpreting BF1; not verified real-world accuracy or safety',
        'boundary_counts':after.boundary.tolist(),'boundary_count_order':['matched_prediction','predicted','matched_reference','reference'],
        'calibration_reference_policy':'confidence_filtered','test_used_for_model_selection':False,'test_used_for_temperature_fit':False,'calibration':calstats},indent=2))
    if args.reuse_export:
        dep=json.loads((RUN/'export/deployment.json').read_text())
        assert dep['source_checkpoint_sha256']==hashlib.sha256((RUN/'best.pt').read_bytes()).hexdigest()
        assert abs(dep['temperature']-temp)<1e-7
        assert dep['sha256']==hashlib.sha256((RUN/'export/gcnet_s_lisbon.onnx').read_bytes()).hexdigest()
        assert dep['onnx_checker']=='passed'
        print('Reused verified unchanged ONNX export',flush=True)
    else:export_model(model,temp,val,args.device)
