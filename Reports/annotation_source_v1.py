...
"""
import argparse,csv,json,sys,time,hashlib
...
def main():
    ...
    policy={'classes':{'0':'path','1':'obstacle','2':'background','255':'ignore'},'teacher_id2label':model.config.id2label,
    ...
    if args.shard_index==0:(out/'label_policy.json').write_text(json.dumps(policy,indent=2))
    start=time.time();done=0
    pending=[r for r in rows if not (out/'metadata'/f"{r['image_id']}.json").exists() or not (out/'dense_proposals'/f"{r['image_id']}.png").exists()]
    for offset in range(0,len(pending),args.batch_size):
        batch=pending[offset:offset+args.batch_size]; originals=[];small=[];alphas=[]
        for r in batch:
            with Image.open(r['source']) as im:
